...
let score = {}; // Changed to object for tracking per category
let timeLeft;
let timerInterval;
let userAnswers;
...
const questionCategories = ["1", "2", "3", "4", "5", "6", "7", "8", "9"];
...
function processResults() {
    const overallScore = Object.values(score).reduce((sum, categoryScore) => sum + categoryScore, 0);
...
function calculateIQ(overallScore) {
    const percentageCorrect = (overallScore / shuffledQuestionIndices.length) * 100;
    let iqEstimate = 100;
...
function resetState() {
  ...
  userAnswers = [];
  originalQuestions = [];
  currentGeneratedQuestion = {category: "0"};  // Initialize to avoid undefined access
  if (timerInterval) {
      clearInterval(timerInterval);
  }
}
